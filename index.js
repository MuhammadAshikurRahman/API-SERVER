...
// POST: নতুন ভেজিটেবল যোগ করার জন্য POST রাউট
...
// PUT: Update a vegetable by ID
app.put('/veg/:id', (req, res) => {
    const id = parseInt(req.params.id); // URL থেকে ভেজিটেবলের ID নেওয়া
    const updatedData = req.body; // ক্লায়েন্ট থেকে পাঠানো নতুন ডেটা
    const vegIndex = vegList.findIndex(item => item.id === id); // লিস্টে ওই আইডির ভেজিটেবল খোঁজা

    vegList[vegIndex] = { ...vegList[vegIndex], ...updatedData }; // পুরানো ডেটার সাথে নতুন ডেটা আপডেট
    res.send(vegList[vegIndex]); // আপডেটেড ভেজিটেবল ক্লায়েন্টকে রেসপন্স করা
});
...
// DELETE: Delete a vegetable by ID
app.delete('/veg/:id', (req, res) => {
    const id = parseInt(req.params.id); // URL থেকে আইডি নেওয়া
    const vegIndex = vegList.findIndex(item => item.id === id); // লিস্টে ওই আইডির ভেজিটেবল খোঁজা

    const deletedVeg = vegList.splice(vegIndex, 1); // সরাসরি ডিলিট করা
    res.send(deletedVeg[0]); // ডিলিট করা ভেজিটেবল ক্লায়েন্টে পাঠানো
});
